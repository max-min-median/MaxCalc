        ...
        while True:
            try:
                ...
                if m := re.match(r'^\s*help\s*$', inp):
                    ...
                    ui.addText("display", ("User-defined Variables", UI["LIGHTBLUE"]["BLACK"]))
                    ui.addText("display", ("──────────────────────", ))
                    for k, v in mainMem.vars.items():
                        if isinstance(v, Function) and k == v.name:
                            ui.addText("display", (f"{v.value()}", UI["LIGHTBLUE"]["BLACK"]))
                        else:
                            ui.addText("display", (k, UI["LIGHTBLUE"]["BLACK"]), (' = ', ), (f"{v.value()}", UI["LIGHTBLUE"]["BLACK"]))
                elif m := re.match(r'^\s*del\s(.*)$', inp):
                    deleted = mainMem.delete(m.group(1))
                    if not deleted:
                        ui.addText("display", ('Variable(s) not found!', UI["LIGHTBLUE"]["BLACK"]))
                    else:
                        ui.addText("display", ("Deleted: ", ))
                        for i, var in enumerate(deleted):
                            if i > 0: ui.addText("display", (', ', ), startNewLine=False)
                            ui.addText("display", (var, UI["LIGHTBLUE"]["BLACK"]), startNewLine=False)
                    ...
                    ui.addText("display", ("frac_max_length", UI["LIGHTBLUE"]["BLACK"]), (' -> ', ), (f"{st.get('frac_max_length')}", UI["LIGHTBLUE"]["BLACK"]))
                elif m := re.match(r'^\s*prec(?:ision)?(?:\s+(\d+))?$', inp):
                    if m.group(1) is not None: st.set("working_precision", int(m.group(1)))
                    ui.addText("display", ("working_precision", UI["LIGHTBLUE"]["BLACK"]), (' -> ', ), (f"{st.get('working_precision')}", UI["LIGHTBLUE"]["BLACK"]))
                elif m := re.match(r'^\s*disp(?:lay)?(?:\s+(\d+))?$', inp):
                    if m.group(1) is not None: st.set("final_precision", int(m.group(1)))
                    ui.addText("display", ("final_precision", UI["LIGHTBLUE"]["BLACK"]), (' -> ', ), (f"{st.get('final_precision')}", UI["LIGHTBLUE"]["BLACK"]))
                    ...
                    else: ui.addText("display", ("Usage: ", ), ("debug [on/off]", UI["LIGHTBLUE"]["BLACK"]))
                    ui.addText("display", ("debug", UI["LIGHTBLUE"]["BLACK"]), (" -> ", ), (f"{st.get('debug')}", UI["LIGHTBLUE"]["BLACK"]))
                elif m := re.match(r'^\s*(?:kb|keyboard)(?:\s+(\w+))?$', inp):
                    flag = {'on':True, 'off':False}.get(m.group(1) if m.group(1) is None else m.group(1).lower(), None)
                    if flag is not None: st.set("keyboard", flag)
                    else: ui.addText("display", ("Usage: ", ), ("keyboard [on/off] (allows use of keyboard module)", UI["LIGHTBLUE"]["BLACK"]))
                    ui.addText("display", ("keyboard", UI["LIGHTBLUE"]["BLACK"]), (" -> ", ), (f"{st.get('keyboard')}", UI["LIGHTBLUE"]["BLACK"]))
                elif m := re.match(r'^\s*(?:quick_exp(?:onents)?)(?:\s+(\w+))?$', inp):
                    flag = {'on':True, 'off':False}.get(m.group(1) if m.group(1) is None else m.group(1).lower(), None)
                    if flag is not None: st.set("quick_exponents", flag)
                    else: ui.addText("display", ("Usage: ", ), ("quick_exp[onents] [on/off]", UI["LIGHTBLUE"]["BLACK"]))
                    ui.addText("display", ("quick_exponents", UI["LIGHTBLUE"]["BLACK"]), (" -> ", ), (f"{st.get('quick_exponents')}", UI["LIGHTBLUE"]["BLACK"]))
                elif inp.strip() == '':
                    continue
                elif m := re.match(r'^\s*(?:=|sto(?:re)? |->)\s*([A-Za-z]\w*)\s*$', inp):
                    if (ans := mainMem.get('ans')) is None:
                        ui.addText("display", ("Variable '", ), ("ans", UI["LIGHTBLUE"]["BLACK"]), ("' does not exist or has been deleted", ))
                    else:
                ...
                ui.addText("display", (repr(e), UI["BRIGHT_RED"]["BLACK"]))
            except RecursionError:
                ui.addText("display", ("RecursionError: Check for infinite recursion in functions.", UI["BRIGHT_RED"]["BLACK"]))
